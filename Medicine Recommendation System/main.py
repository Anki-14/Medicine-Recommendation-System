...
#load database:
sym_des = pd.read_csv("Datasets1/symtoms_df.csv")
precautions = pd.read_csv("Datasets1/precautions_df.csv")
workout = pd.read_csv("Datasets1/workout_df.csv")
description = pd.read_csv("Datasets1/description.csv")
medications = pd.read_csv("Datasets1/medications.csv")
diets = pd.read_csv("Datasets1/diets.csv")
...
def get_predicted_value(patient_symptoms):
    ...
    for item in patient_symptoms:
        ...
        if item.strip() != '' and item in symptoms_dict:
            input_vector[symptoms_dict[item]] = 1
        else:
            print(f"Skipping invalid symptom: '{item}'")
...
def predict():
    if request.method == 'POST':
        ...
        # Split the user's input into a list of symptoms (assuming they are comma-separated) # itching,skin_rash,
        ...
        return render_template('index.html', predicted_disease=predicted_disease, dis_des=desc, pre=my_pre, med=med,
                               dis_diet=die, dis_wrkout=wrkout)
